...
function CompareTable({ title, guide, rows, showVirtual = false, inv = 10000 }) {
  const baseline = rows[0]?.summary;
  const metrics = showVirtual
    ? [...METRICS_BASE,
        { key: "virtualBuyCount", label: "샀다치고", format: v => `${v}회`, better: "lower" },
        { key: "extraBuyNeeded", label: "추가 필요 투자금", format: v => v > 0 ? `+$${Math.round(v).toLocaleString()} (${Math.round(v / inv * 100)}%)` : "-", better: "lower" },
      ]
    : METRICS_BASE;
      ...
      {results && (
        <>
          ...
          <CompareTable
            ...
            showVirtual
            inv={Number(investment) || DEFAULT_USD}
          />
          <CompareTable
            title="업다운 매도 조건"
